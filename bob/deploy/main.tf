...
#     Environment="HOME=/home/cloudservice"
#     ExecStartPre=/usr/bin/docker-credential-gcr configure-docker
#     ExecStart=/usr/bin/docker run --rm -u 0 --device=/dev/tpm0:/dev/tpm0 --name=mycloudservice gcr.io/${var.project_id}/tokenclient@${var.image_hash} --useTPM --address ${var.ts_address}:50051 --tsAudience ${var.ts_audience} --useALTS --doAttestation --exchangeSigningKey --v=20 -alsologtostderr
#     ExecStop=/usr/bin/docker stop mycloudservice
#     ExecStopPost=/usr/bin/docker rm mycloudservice
